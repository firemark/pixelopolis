...
DRAW_BUILDER_OBJS += series_utils void board_obj union
DRAW_BUILDER_OBJS += filler series square_fence bsp_tree
DRAW_BUILDER_OBJS += cube cone cylinder dome pyramid triangle
DRAW_BUILDER_OBJS += wall floor texture
CSS_DRAW_OBJS  = component base texture
CSS_DRAW_OBJS += cube cone cylinder dome pyramid triangle
CSS_DRAW_OBJS += series board_obj void
TESTS = hash css_eval
...
ifeq ($(VERSION),PROD)
FLAGS = -O3 -g -Wall -std=gnu11
else
FLAGS = -g -Wall -std=gnu11 #-fsanitize=address,undefined
endif
