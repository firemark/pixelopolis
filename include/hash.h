...
#define hash_iter(obj, map) \
    ...
    for(obj = map->items[0]; HASH_CONCAT(hash_iter_i, __LINE__) < map->max_size; \
...
#define hash_iter_values(obj, map) \
    ...
    for(HASH_CONCAT(hash_item, __LINE__) = map->items[0]; HASH_CONCAT(hash_iter_i, __LINE__) < map->max_size; \
            HASH_CONCAT(hash_item, __LINE__) = map->items[HASH_CONCAT(hash_iter_i, __LINE__)++]) \
        if (obj = HASH_CONCAT(hash_item, __LINE__) ? HASH_CONCAT(hash_item, __LINE__)->value : NULL)
...
#define hash_iter_keys(obj, map) \
    ...
    for(HASH_CONCAT(hash_item, __LINE__) = map->items[0]; HASH_CONCAT(hash_iter_i, __LINE__) < map->max_size; \
            HASH_CONCAT(hash_item, __LINE__) = map->items[HASH_CONCAT(hash_iter_i, __LINE__)++]) \
        if (obj = HASH_CONCAT(hash_item, __LINE__) ? HASH_CONCAT(hash_item, __LINE__)->key : NULL)
