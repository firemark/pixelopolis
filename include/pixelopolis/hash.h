...
#define _hash_iter(map)                                                      \
    ...
    for (HASH_LINE(hash_iter_i) = 0; HASH_LINE(hash_iter_i) < map->max_size; \
         HASH_LINE(hash_iter_i)++)

#define _hash_item(map) map->items[HASH_LINE(hash_iter_i)]

#define hash_iter(obj, map) _hash_iter(map) if ((obj = _hash_item(map)))

#define hash_iter_values(obj, map)                                \
    struct HashStrItem* HASH_LINE(hash_item);                     \
    _hash_iter(map) if ((HASH_LINE(hash_item) = _hash_item(map)), \
