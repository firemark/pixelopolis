...
#define MAKE_SELECTOR_HELPER(helper, prop) { \
    ...
    .selector=css_find_selector_prop(helper->rule->rule, prop), \
...
enum Justify builder_get_justify(struct RuleWithParent* rule, char* key, const enum JustifyIndex index);