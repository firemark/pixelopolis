...
void css_free_obj(struct Obj* obj) {
    if (obj->type & OBJ_BINARY_OP) {
        ...
        switch (obj->type) {
            case OBJ_FUNC:
                css_free_func_obj((struct FuncObj*)obj->value);
                break;
            case OBJ_RULE:
                css_free_rule_selector((struct RuleSelector*)obj->value);
                break;
            default:
                free(obj->value);
                break;
...
void css_free_rule_selector(struct RuleSelector* selector) {
    ...
    if (selector->klasses) {
        char* klass;
        css_iter (klass, selector->klasses) { free(klass); }
    }
...
void css_free_objs(struct Obj** objs) {
    return;  // currently todo - without gargabage collector is hard to delete
             // sth
    struct Obj* obj;
    css_iter (obj, objs) { css_free_obj(obj); }
    free(objs);
}

void css_free_rule(struct Rule* rule) {
    struct Obj** objs;
    hash_iter_values(objs, rule->props) { css_free_objs(objs); }
