...
void _add_margin_to_tex_objs(struct ShiftTexPair** pairs, size_t start_index, size_t end_index,
                             int margin) {
    size_t index;
...
void _align_tex_objs(struct Helper* helper, struct ShiftTexPair** pairs, size_t size,
                     int start_width, int end_width, char with_left, char with_right) {
    if (size == 0) {
        return;
    }
...
void _append_objs_to_floor(struct Helper* helper, struct FloorObj* floor, int end_width) {
    ...
    for (;;) {
        ...
        if (shift + middle_width > end_width)
            break;  // TODO - remove created tex-obj and to avoid memory leak
