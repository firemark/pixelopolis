...
struct DrawObj* builder_build_pyramid(struct Helper* helper) {
    ...
    const int south_length = sqrt(basic.depth * basic.depth / 4.0 + basic.height * basic.height);
    obj->south_wall = builder_build_wall(&wall_helper, basic.width, south_length);

    const int east_length = sqrt(basic.width * basic.width / 4.0 + basic.height * basic.height);
