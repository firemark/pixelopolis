...
static void _fill_body(struct BoardObj* obj, struct Helper* helper,
...
static struct DrawObj* _make_edge(struct Helper* helper, const char is_vertical,
                                  const int primal_rotate, const struct SquareFenceData* data) {
    struct BasicObj* parent_basic = &helper->parent->basic;
    const int edge_size = data->edge_size;
...
static void _fill_edges_and_corners(struct BoardObj* obj, struct Helper* helper,
                                    const struct SquareFenceData* data) {
