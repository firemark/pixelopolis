...
struct TexObj* builder_texture_build_default(struct TextureHelper* helper) {
    ...
    obj->texture = _find_texture_in_rule(helper->rule);
    obj->normal_map = _find_bump_map_in_rule(helper->rule);  // || _find_normal_map_in_rule(rule);
    obj->color = _find_color_in_rule(helper->rule);
    obj->texture_cache = true;
    obj->normal_map_cache = true;
